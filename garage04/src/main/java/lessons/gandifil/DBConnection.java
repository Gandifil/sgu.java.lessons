...
public class DBConnection {
    ...
    private DBConnection(){
        try{
            sessionFactory = new Configuration().configure("/lessons/gandifil/hibernate.cfg.xml").buildSessionFactory();
            generate();
    ...
    public List<Car> getCars(){
        try{
            return s.createQuery("from lessons.gandifil.Car").list();
        }
        catch (Exception e){
    ...
    public List<Garage> getGarageCars(){
        try{
            return s.createQuery("from lessons.gandifil.Garage").list();
        }
        catch (Exception e){
            System.err.println("Statement 'getCars' Failed");
            e.printStackTrace();
